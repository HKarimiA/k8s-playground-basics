...
[source%nowrap,bash]
----
  kubectl create configmap config --from-file=service/greeting/config/config.yaml
  # or 
  kubectl apply -f yaml-solutions/greeting-config.yaml
----
  
* Use the probes rest endpoints
** Access to the probe endpoints is only possible from within the k8s cluster

**Solution:** see yaml-solutions/greeting-deployment.yaml
...
----
  ...
  kubectl apply -f yaml-solutions/greeting-deployment.yaml
  
  # cpu autoscaling
  kubectl autoscale deployment greeting-deployment --name=greeting-hpa-cpu --cpu-percent=50 --min=1 --max=10
  # or 
  kubectl apply -f yaml-solutions/greeting-autoscaling-cpu.yaml
  
  # memory autoscaling
  kubectl apply -f yaml-solutions/greeting-autoscaling-memory.yaml
----
...
----
  ...
  kubectl apply -f yaml-solutions/greeting-service.yaml
  
  # greeting-ingress.yaml
  kubectl apply -f yaml-solutions/greeting-ingress.yaml
...
[source%nowrap,bash]
----
  # Deploy your base configuration to Kubernetes (for example as development environment)
  kubectl apply -k yaml-solutions/base/
  
  # Deploy your base configuration to Kubernetes with some changes (for example as production environment)
  kubectl apply -k yaml-solutions/overlays/production/
